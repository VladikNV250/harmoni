...
interface IEditMenu {
    /** Object of playlist that can be edited. */
...
/**
 * @component EditMenu
 * @description Modal dialog that allows editing the name, description of a playlist or change cover image.
 */
export const EditMenu: FC<IEditMenu> = ({ playlist, setIsOpen, isOpen }) => {
    ...
    /** Uploaded image must be JPEG format and have size < 256kb */
    ...
    const submitChanges = async () => {
        try {
            ...
            if (editData.previewImage !== playlist.images?.[0]?.url && editData.previewImage) {
                 ...
                 * because the Web API expects only the raw base64 string without metadata.   
                 */
                const base64Image = editData.previewImage.split(',')[1];
